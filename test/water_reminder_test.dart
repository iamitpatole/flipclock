...
void main() {
  ...
  group('Water Reminder Settings & Timer Tests', () {
    ...
    test('Default values for water reminder are correct', () {
      ...
      expect(
        settingsProvider.settings.waterReminderIntervalSeconds,
        equals(3600),
      );
    ...
    test('Enabling water reminder sets next trigger timestamp', () async {
      ...
      expect(
        settingsProvider.settings.nextWaterReminderTimestamp,
        greaterThan(0),
      );

      final expectedTriggerTime =
          beforeTime +
          settingsProvider.settings.waterReminderIntervalSeconds * 1000;
    ...
    test(
      'Changing interval calculates new reminder timestamp if enabled',
      () async {
        await settingsProvider.setWaterReminderEnabled(true);
    ...
    test(
      'Triggering reminder advances next timestamp and shows animation on dismiss',
      () async {
        await settingsProvider.setWaterReminderEnabled(true);
        ...
        final expectedNextTime =
            DateTime.now().millisecondsSinceEpoch +
            settingsProvider.settings.waterReminderIntervalSeconds * 1000;
        expect((nextTimestamp - expectedNextTime).abs(), lessThan(500));
      },
    );
  });
}
